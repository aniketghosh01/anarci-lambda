...
def get_assay_run(benchling_sdk, schema_id, notebook_id, filename):
    iterator=benchling_sdk.assay_runs.list(schema_id=schema_id)
    assay_run=None
...
def register_entities(benchling_sdk, type, data, entity_ids, logger):
    ...
    def resolve_msg_vr_ids(benchling_sdk, result, schema_id):    
        ...
        for i in range(len(result["errors"])):
            ...
            query=benchling_sdk.custom_entities.list(name=vr_name,schema_id=schema_id)
            vr_id=query.first()
            new_row=pd.DataFrame({"index": [result["errors"][i]["index"]],"vr_id": [vr_id.id]})
            res_df=pd.concat([res_df, new_row], ignore_index=True)
        return res_df 
...
logg.info([orientation_vhvl,linker,notebook_id])

vh_list=[]
vl_list=[]
for seq in df["seq"]:
    vh,vl=split_scfv(seq,linker,logg,orientation_vhvl)
    if vh == '':
...
df=register_entities(benchling_sdk,"vh",df,entity_ids,logg)
logg.info(df.head())

df=register_entities(benchling_sdk,"vl",df,entity_ids,logg)
logg.info(df.head())

df=register_entities(benchling_sdk,"vr",df,entity_ids,logg)
logg.info(df.head())
...
task = benchling.process_output(run=assay_run.id,name="ScFv Output File",blob=blob.id)